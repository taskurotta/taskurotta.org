...
При обращении к методу promise.get() выбрасывается исключение Fail. Тип изначального исключения можно узнать узнать так: e.getType(). Нужно помнить что все асинхронные задачи, поставленные до вылета исключения из метода get(), будут поставленны в очереди и выполены (особенность текущей версии). 

TODO:
Далее описываем то что происходит на гранцах одних фич с лругими. Например:

- Если указан RetryPolicy, то будет его выполнения и только в случае неудачи полетит FailPromise координатору
- Если координатор положит Unsafe Promise как аргумент актеру, ТО. Это происходит потому ЧТО. Для исправлении ситуации нужно сделать ЧТО. Предотвратить ситуацию можно если делать ЧТО
- ну и т.д.
