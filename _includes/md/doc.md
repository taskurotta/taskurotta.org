...
При обращении к методу promise.get() выбрасывается исключение Fail. Тип изначального исключения можно узнать узнать так: e.getType(). Нужно помнить что все асинхронные задачи, поставленные до обращения к методу get() будут поставленны и обработаны.
